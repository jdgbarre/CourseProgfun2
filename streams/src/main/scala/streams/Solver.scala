...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, listmove) #:: remaining => {
      val neighbors = newNeighborsOnly(neighborsWithHistory(block, listmove), explored + block)

      neighbors ++ from(remaining ++ neighbors, explored + block)
    }
  }
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    case (_, listMove) #:: _ => listMove.reverse
  }
}
